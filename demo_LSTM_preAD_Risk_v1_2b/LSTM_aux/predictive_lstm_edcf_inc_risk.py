...
def predictive_lstm_edcf_risk(model, X_test, mask_dyn_test, y_test, diag_init_test, scaler=None):
...
def predict_full_trajectory_risk(model, x_past, mask_dyn_test, diag_init, n_future, static_indices, device='cpu'):
    """
    ...
    with torch.no_grad():
        for _ in range(n_future):
            ...
            pred_meas, pred_dx, _ = model(x_seq, mask_dyn_test_t, diag_init)
            mask_dyn_test_t[:,:]=1
